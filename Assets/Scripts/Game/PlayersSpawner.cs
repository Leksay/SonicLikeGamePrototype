...
namespace Game
{
    public class PlayersSpawner : MonoBehaviour
    {
        ...
        private void SpawnEnemy(int road, List<int> ids)
        {
            ...
            ids.RemoveAt(0);
            enemy.GetComponent<IOpponentMover>().SetRoad(road, Locator.GetObject<LevelHolder>()._startPercent);
        }

        private void SpawnPlayer(int roadId)
        {
            var player = Instantiate(_playerPrefab).GetComponent<IMover>();
            player.SetStartRoad(roadId, Locator.GetObject<LevelHolder>()._startPercent);
        }
    }
}
